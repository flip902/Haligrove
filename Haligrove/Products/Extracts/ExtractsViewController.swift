...
class ExtractsViewController: UIViewController, UITableViewDelegate, UITableViewDataSource, ExtractsFavoriteDelegate {
    ...
    var extractsTableView: UITableView!
    ...
    var reuseIdentifier = "extractCell"
    let urlString = "http://app.haligrove.com/extractsData.json"
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        ApiService.shared.fetchJson(from: urlString) { [weak self](data: [Product]) in
            ...
            self?.extractsTableView.reloadData()
        }
    }
    
    // MARK: - Class Methods
    func setupTableView() {
        navigationItem.title = "Extracts"
        extractsTableView = UITableView(frame: CGRect(x: 0, y: 0, width: view.frame.width, height: view.frame.height), style: .plain)
        extractsTableView.register(ExtractsCell.self, forCellReuseIdentifier: reuseIdentifier)
        extractsTableView.backgroundColor = #colorLiteral(red: 0.2549019754, green: 0.2745098174, blue: 0.3019607961, alpha: 1)
        extractsTableView.dataSource = self
        extractsTableView.delegate = self
        extractsTableView.separatorStyle = .none
        self.view.addSubview(extractsTableView)
        ...
        extractsTableView.anchor(top: view.safeAreaLayoutGuide.topAnchor, right: view.safeAreaLayoutGuide.rightAnchor, bottom: view.safeAreaLayoutGuide.bottomAnchor, left: view.safeAreaLayoutGuide.leftAnchor, paddingTop: 0, paddingRight: 0, paddingBottom: 0, paddingLeft: 0, width: 0, height: 0)
    }
    
    func setup() {
        itemHeight = Array(repeating: closeHeight, count: rowsCount)
        extractsTableView.estimatedRowHeight = closeHeight
        extractsTableView.rowHeight = UITableViewAutomaticDimension
    }
    
    override func didRotate(from fromInterfaceOrientation: UIInterfaceOrientation) {
        extractsTableView.reloadData()
    }
    
    // MARK: Delegate Methods
    func didTapExtractFavoritesButton(in cell: ExtractsCell) {
        guard let indexTapped = extractsTableView.indexPath(for: cell) else { return }
    ...
    func tableView(_ tableView: UITableView, willDisplay cell: UITableViewCell, forRowAt indexPath: IndexPath) {
        guard case let cell as ExtractsCell = cell else { return }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = extractsTableView.dequeueReusableCell(withIdentifier: reuseIdentifier, for: indexPath) as! ExtractsCell
        var product: Product
        cell.delegate = self
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        let cell = tableView.cellForRow(at: indexPath) as! ExtractsCell
        if cell.isAnimating() {
            return
        }
        var duration = 0.0
